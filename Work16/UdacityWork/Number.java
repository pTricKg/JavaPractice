...
public class Number
{
    ...
    public int countMatchingDigits(int digitToMatch)
    {
        ...
        while(temp > 0)
        {
            int digit = (int)(temp % 10); // cast int on long
            System.out.println("Pop this " + digit);
            if (digit == digitToMatch)
            {
                count++;
                System.out.println("And we have a winner: " + digitToMatch);
            }
            temp = temp / 10; // pop off last digit of long for comparison
            System.out.println("After pop " + temp);
